...
[![npm](https://img.shields.io/npm/v/peasy-pdf-embed)](https://www.npmjs.com/package/peasy-pdf-embed)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![Zero Dependencies](https://img.shields.io/badge/dependencies-0-brightgreen)](https://www.npmjs.com/package/peasy-pdf-embed)
[![Size](https://img.shields.io/badge/gzipped-<8KB-green)](https://bundlephobia.com/package/peasy-pdf-embed)

Embed **PeasyPDF** widgets on any website or blog. PDF tools, conversion, compression, merging, and splitting. Each widget is completely isolated via **Shadow DOM**, requires **zero dependencies**, and supports **3 built-in themes** (light, dark, sepia). The self-executing script finds all `data-peasy-pdf` elements on the page and renders interactive widgets automatically, including support for dynamically added elements in single-page applications via MutationObserver.

PeasyPDF is part of the [Peasy Tools](https://peasytools.com) ecosystem, providing 11 widget types for embedding file format information, tool cards, conversion guides, glossary terms, search boxes, and interactive tools directly into your pages.

> **Try the interactive widget builder at [widget.peasypdf.com](https://widget.peasypdf.com)** -- build, preview, and copy embed code for any widget type.

<p align="center">
  <img src="demo.gif" alt="peasy-pdf-embed demo -- embed peasypdf widgets with Shadow DOM isolation and theme support" width="800">
</p>
...
That's it. The script will find all `data-peasy-pdf` elements, fetch data from the PeasyPDF API, and render each widget with full Shadow DOM style isolation.

## Widget Types

PeasyPDF supports 11 widget types -- 11 common types available on all Peasy sites.

| Type | Description | Usage |
|------|-------------|-------|
| `format` | File format info card with specs, MIME type, and use cases | `<div data-peasy-pdf="format" data-slug="..."></div>` |
| `tool` | Tool card with description and direct launch link | `<div data-peasy-pdf="tool" data-slug="..."></div>` |
| `convert` | Conversion card showing input/output format pair | `<div data-peasy-pdf="convert" data-slug="..."></div>` |
| `compare` | Side-by-side comparison of two formats or tools | `<div data-peasy-pdf="compare" data-slug="..."></div>` |
| `glossary` | Glossary term tooltip with definition on hover | `<div data-peasy-pdf="glossary" data-slug="..."></div>` |
| `gallery` | Grid gallery of available tools for a category | `<div data-peasy-pdf="gallery" data-slug="..."></div>` |
| `guide` | Guide/tutorial card with summary and read-more link | `<div data-peasy-pdf="guide" data-slug="..."></div>` |
| `usecase` | Use-case card showing when and why to use a tool | `<div data-peasy-pdf="usecase" data-slug="..."></div>` |
| `badge` | Inline badge showing format type or tool category | `<div data-peasy-pdf="badge" data-slug="..."></div>` |
| `search` | Search box with autocomplete for tools and formats | `<div data-peasy-pdf="search" data-slug="..."></div>` |
| `interactive` | Embedded interactive tool via iframe | `<div data-peasy-pdf="interactive" data-slug="..."></div>` |
...
|-----------|--------|---------|-------------|
| `data-peasy-pdf` | format, tool, convert, compare, glossary, gallery, guide, usecase, badge, search, interactive | *required* | Widget type to render |
...
<div data-peasy-pdf="format" data-slug="pdf" data-theme="light"></div>

<!-- Dark theme -- dark background, light text -->
<div data-peasy-pdf="format" data-slug="pdf" data-theme="dark"></div>

<!-- Sepia theme -- warm, paper-like appearance -->
<div data-peasy-pdf="format" data-slug="pdf" data-theme="sepia"></div>

<!-- Auto theme -- follows prefers-color-scheme -->
<div data-peasy-pdf="format" data-slug="pdf" data-theme="auto"></div>
```

Each theme sets CSS custom properties inside the Shadow DOM. The accent color (`#EF4444`) is consistent across all themes, providing brand recognition for PeasyPDF.
...
```html
<script type="module">
  import 'https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.esm.js';
</script>
```
...
```html
<!-- PeasyPDF format info card with light theme -->
<div data-peasy-pdf="format" data-slug="pdf" data-theme="light"></div>
<script src="https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.min.js"></script>
```
...
```html
<!-- PeasyPDF tool card -->
<div data-peasy-pdf="tool" data-slug="pdf" data-theme="light"></div>
<script src="https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.min.js"></script>
```
...
```html
<!-- Search across PeasyPDF tools and formats -->
<div data-peasy-pdf="search"
  data-placeholder="Search PeasyPDF tools..."
  data-theme="light">
</div>
<script src="https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.min.js"></script>
```
...
```html
<p>This file is in <div data-peasy-pdf="badge" data-slug="pdf"></div> format.</p>
<script src="https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.min.js"></script>
```
...
```html
<!-- Embed an interactive PeasyPDF tool -->
<div data-peasy-pdf="interactive"
  data-slug="pdf/example-tool"
  data-height="500"
  data-theme="light">
</div>
<script src="https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.min.js"></script>
...
> **Note:** The `data-slug` value uses the format `category-slug/tool-slug`. Find available tools at [peasypdf.com/api/v1/tools/](https://peasypdf.com/api/v1/tools/).
...
```html
<div data-peasy-pdf="glossary" data-slug="lossless" data-theme="light"></div>
<script src="https://cdn.jsdelivr.net/npm/peasy-pdf-embed@1/dist/embed.min.js"></script>
```
...
- **CORS**: PeasyPDF API has CORS enabled for all origins
...
- **Powered by**: Each widget includes a subtle "PeasyPDF" backlink in the footer

## Learn More

Visit [peasypdf.com](https://peasypdf.com) for the full PeasyPDF experience.

- **All Tools**: [peasypdf.com](https://peasypdf.com)
- **Formats**: [peasypdf.com/formats/](https://peasypdf.com/formats/)
- **Glossary**: [peasypdf.com/glossary/](https://peasypdf.com/glossary/)
- **Guides**: [peasypdf.com/guides/](https://peasypdf.com/guides/)
- **API Documentation**: [peasypdf.com/developers/](https://peasypdf.com/developers/)
- **OpenAPI Spec**: [peasypdf.com/api/openapi.json](https://peasypdf.com/api/openapi.json)
- **Widget Builder**: [widget.peasypdf.com](https://widget.peasypdf.com)
- **npm Package**: [npmjs.com/package/peasy-pdf-embed](https://www.npmjs.com/package/peasy-pdf-embed)
